  ...
  <head>
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      /* Intro */
      .intro { background: ${PALETTE.bgLight}; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 36px; }
      .intro-logo { width: 360px; height: 360px; object-fit: contain; filter: drop-shadow(0 24px 60px rgba(217, 138, 90, 0.32)); }
      .intro-wordmark { font-family: "Orbitron", sans-serif; font-weight: 600; font-size: 64px; letter-spacing: 0.32em; text-transform: uppercase; color: ${PALETTE.ink}; }
      /* Outro */
  ...
  <body>
    <div id="root" data-composition-id="main" data-start="0" data-duration="${totalDur}" data-width="1080" data-height="1920">
      ...
      <div id="intro" class="clip intro" data-start="0" data-duration="${introDur}" data-track-index="0">
        <img src="assets/logo/logo.png" alt="logo" class="intro-logo" />
        <div class="intro-wordmark">uygundavet</div>
      </div>
...
${scenes.map((s, i) => {
  ...
  return `      <div id="${s.id}" class="clip" data-start="${s.start}" data-duration="${s.dur}" data-track-index="1"${styleAttr}>${sceneInner(s)}</div>`;
    ...
    <script>
      ...
      // Intro
      tl.fromTo("#intro", { opacity: 0 }, { opacity: 1, duration: 0.3 }, 0);
      tl.from("#intro .intro-logo", { scale: 0.85, opacity: 0, duration: 0.5, ease: "back.out(1.6)" }, 0.1);
      tl.from("#intro .intro-wordmark", { y: 30, opacity: 0, duration: 0.45, ease: "power3.out" }, 0.3);
