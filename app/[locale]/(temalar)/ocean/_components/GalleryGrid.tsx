  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-6 h-6 border-2 border-[#2d8b8b]/30 border-t-[#2d8b8b] rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {wedding.hasGallery && (
        ...
        >
          <div className="bg-[#f1faee] rounded-[1.25rem] border border-[#2d8b8b]/20 shadow-sm p-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
              <div className="relative flex-1">
                <UserIcon
                  size={16}
                  className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#2d8b8b]"
                />
                <input
                  type="text"
                  value={uploaderName}
                  ...
                  className="w-full h-11 rounded-xl border border-[#2d8b8b]/25 bg-white pl-9 pr-4 text-base sm:text-sm text-[#1a2332] placeholder:text-[#1a2332]/40 focus:border-[#2d8b8b] focus:outline-none focus:ring-2 focus:ring-[#2d8b8b]/15 transition-all font-sans"
                />
              </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  isUploading
                    ? "bg-[#2d8b8b]/20 text-[#2d8b8b]/50 cursor-not-allowed"
                    : "bg-[#1a2332] text-[#f1faee] hover:bg-[#2d8b8b] cursor-pointer"
                )}
              >
                {isUploading ? (
                  <>
                    <div className="w-3.5 h-3.5 border-2 border-[#2d8b8b]/30 border-t-[#2d8b8b] rounded-full animate-spin" />
            ...
            <p className="font-sans text-[10px] text-[#1a2332]/45 mt-2 text-center">
              {t("galleryUploadInfo")}
            </p>
          </div>
        </motion.div>
      ...
      {photos.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 2xl:grid-cols-8 gap-2">
          {photos.map((photo, index) => (
            <motion.div
              ...
              className="group relative aspect-[4/3] overflow-hidden rounded-[1rem] cursor-pointer bg-[#e6eef0] hover:shadow-[0_12px_30px_-12px_rgba(26,35,50,0.45)] transition-shadow duration-300"
            ...
            >
              ...
              />
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0d1620]/80 to-transparent pt-6 pb-2 px-2.5">
                <p className="font-sans text-[10px] text-[#f1faee]/90 truncate">
                  {photo.uploader}
                </p>
              </div>
            </motion.div>
        ...
        >
          <div className="text-center py-16 space-y-3">
            <div className="w-14 h-14 rounded-full bg-[#2d8b8b]/10 border border-[#2d8b8b]/25 flex items-center justify-center mx-auto mb-4">
              <CameraIcon size={24} className="text-[#2d8b8b]/60" />
            </div>
            <p className="font-sans text-sm text-[#1a2332]/55">
              {t("galleryEmpty")}
            </p>
            <p className="font-sans text-xs text-[#1a2332]/35">
      ...
      <AnimatePresence>
        {selectedPhoto && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#0d1620]/95 backdrop-blur-sm flex items-center justify-center p-4"
            onClick={() => setSelectedPhoto(null)}
          >
            <button
              onClick={() => setSelectedPhoto(null)}
              aria-label="Kapat"
              className="absolute top-4 right-4 w-10 h-10 rounded-full bg-[#2d8b8b]/25 border border-[#a8dadc]/30 flex items-center justify-center text-[#f1faee]/85 hover:text-[#a8dadc] transition-colors z-10"
            >
              <CloseIcon size={20} />
            </button>
            ...
            <div className="absolute bottom-6 left-0 right-0 text-center">
              <p className="font-sans text-sm text-[#f1faee]/75">{selectedPhoto.uploader}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
