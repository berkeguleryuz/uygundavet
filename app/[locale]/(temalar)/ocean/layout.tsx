...
import { OceanNav } from "./_components/OceanNav";
import { OceanFooter } from "./_components/OceanFooter";

const INVITE_CODE = process.env.OCEAN_INVITE_CODE || process.env.Grow_INVITE_CODE || "";
...
const getWeddingData = cache(async (): Promise<WeddingData | null> => {
  ...
  try {
    ...
    return {
      ...
      selectedTheme: order?.selectedTheme || "ocean",
...
export default async function OceanLayout({ children }: { children: React.ReactNode }) {
  let data: WeddingData | null = null;
  try {
    ...
    return (
      <div className="min-h-svh flex flex-col items-center justify-center bg-[#0d1620] px-6 text-center">
        ...
        <a href="" className="font-sans text-sm font-bold tracking-[0.15em] uppercase bg-[#2d8b8b] text-[#f1faee] rounded-full px-8 py-3 hover:bg-[#3aa0a0] transition-colors">
          Tekrar Dene
        </a>
      </div>
    );
  ...
  if (!data) {
    return (
      <div className="min-h-svh flex flex-col items-center justify-center bg-[#0d1620] px-6 text-center">
        <h2 className="font-merienda text-2xl text-[#a8dadc] mb-4">Davetiye bulunamadı</h2>
        <p className="font-sans text-sm text-[#f1faee]/50">Bu davetiye artık mevcut değil veya bağlantı geçersiz.</p>
      </div>
    );
  }
