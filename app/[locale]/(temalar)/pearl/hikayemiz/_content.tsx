...
export function HikayemizContent() {
  ...
  return (
    <div className="min-h-svh bg-[#f7f4ef] pt-24 pb-16 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        ...
        >
          <p className="font-sans text-xs tracking-[0.3em] uppercase text-[#b8a088] mb-3">
            {t("storyLabel")}
          </p>
          <h1 className="font-merienda text-4xl md:text-5xl bg-gradient-to-r from-[#b8a088] via-[#c4a296] to-[#8a7d6d] bg-clip-text text-transparent">
            {brideFirst} & {groomFirst}
          </h1>
          <p className="font-sans text-sm text-[#1c1917]/50 mt-4 max-w-lg mx-auto leading-relaxed">
            {t("storyText")}
          </p>
        </motion.div>
              ...
              >
                ...
                <div className={`md:w-1/2 ${idx % 2 !== 0 ? "md:text-right" : ""}`}>
                  <div className={`${idx % 2 !== 0 ? "md:border-r-4 md:border-l-0 md:pr-6" : "border-l-4 pl-6"} ${accentBorders[idx % accentBorders.length]} ${idx % 2 !== 0 ? accentBorders[idx % accentBorders.length].replace("border-l-", "md:border-r-") : ""}`}>
                    <span className="inline-block font-sans text-xs tracking-[0.2em] uppercase text-[#b8a088] mb-3 font-medium">
                      {milestone.date}
                    </span>
                    <h3 className="font-merienda text-2xl text-[#1c1917] mb-3">
                      {milestone.title}
                    </h3>
                    <p className="font-sans text-sm text-[#1c1917]/60 leading-[1.8]">
        ...
        >
          <div className="w-24 h-1 rounded-full bg-gradient-to-r from-[#b8a088] via-[#c4a296] to-[#8a7d6d]" />
        </motion.div>
      </div>
    </div>
  );
